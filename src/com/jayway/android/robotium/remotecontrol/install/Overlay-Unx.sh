...
#This script supply an inter-active way to help user overlay an existing RobotiumRC.
#User can decide to overlay or not, and the overlay directory.

#12 JUL, 2013  Carl Nagle

if ( test $# = 0 ) ; then
  echo " "
  echo "Usage: Overlay-Unx.sh AndroidSDKPath"
...
then
  ...
  installSAFS=1
  echo "The default RobotiumRC installation directory is $SAFSDirectory, is that where RobotiumRC is installed? [Y|N], default is Y?"
  read tmp
